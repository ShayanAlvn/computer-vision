...
def convolve(image, K):
    ...
    #exch (x, y)-coordinate from left-to-right and top-to-bottom
    for y in np.arange(pad, iH + pad):
        for x in np.arange(pad, iW + pad):
            ...
            #cooordinate of the output image
...
#construct the Sobel y-aixs kernel
sobelY = np.array((
    [-1, -2, -1],
    [0, 0, 0],
    [1, 2, 1]), dtype="int")
...
for (kernelName, K) in kernelBank:
    ...
    #'convole' function and OpenCV's 'filter2D' function
